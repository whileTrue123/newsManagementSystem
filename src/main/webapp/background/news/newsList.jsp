...
function newsDelete(newsId){
	if(confirm("确认要删除这条新闻吗？")){
		$.post("newsDelete.action",{newsId:newsId},
			function(delFlag){
				var flag=eval('('+delFlag+')');
				if(flag){
					alert("删除成功!");
					window.location.href="${pageContext.request.contextPath}/newsBackList.action";
				}else{
					alert("删除失败");
				}
			}
...
<div class="data_list backMain">
	...
	<div class="search_content" style="vertical-align: middle;">
		<form action="${pageContext.request.contextPath}/newsBackList.action" method="post">
		...
		<table class="table table-hover table-bordered">
			...
			<c:forEach var="newsBack" items="${newsBackList }" varStatus="status">
				<tr>
					...
					<td><button class="btn btn-mini btn-info" type="button" onclick="javascript:window.location.href='newsPreSave.action?newsId=${newsBack.newsid}'">修改</button>&nbsp;<button class="btn btn-mini btn-danger" type="button" onclick="newsDelete(${newsBack.newsid})">删除</button></td>
				</tr>
			</c:forEach>
		</table>
	</div>
