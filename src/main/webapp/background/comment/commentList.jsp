...
<script type="text/javascript">
	...
	function commentsDelete(){
		...
		if(confirm("确认要删除这些评论吗？")){
			$.post("commentDelete.action",{commentIds:commentIds},
				function(result){
					var result=eval('('+result+')');
					if(result.success){
						alert("成功删除"+result.delNums+"条数据");
						window.location.href="${pageContext.request.contextPath}/commentBackList.action";
					}else{
	...
	function commentDelete(commentId){
		if(confirm("确认要删除这条评论吗？")){
			$.post("commentDelete.action",{commentIds:commentId},
				function(result){
					var result=eval('('+result+')');
					if(result.success){
						alert("删除成功！");
...
<div class="data_list backMain">
	...
	<div class="search_content" style="vertical-align: middle;padding-right: 20px;">
		...
		<div style="float: right;">
			<form action="${pageContext.request.contextPath}/commentBackList.action" method="post">
		...
		<table class="table table-hover table-bordered">
			...
			<c:forEach var="commentBack" items="${CommentBackList }" varStatus="status">
				<tr>
					...
					<td><a title="${commentBack.newsTitle }" target="_blank" href="newsShow.action?newsId=${commentBack.newsid }">${fn:substring(commentBack.newsTitle,0,15) }...</a></td>
