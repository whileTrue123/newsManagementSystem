			...
			<DIV style="width: 330px; height: 228px;" class="tuhuo">
				<A href="" target="_blank"><IMG style="width: 330px; height: 208px;" id="fou_img" src=""></A>
					<c:forEach var="imageNews" items="${imageNewsList}">
						<A href="newsShow.action?newsId=${imageNews.newsid }"> 
							<IMG style="display: none;" class="tu_img" src="${pageContext.request.contextPath}/userImage/${imageNews.imagename }" width="330" height="208" />
						</A>
					</c:forEach>
					<c:forEach var="imageNews" items="${imageNewsList}">
						<P style="height: 20px;" class="tc"><A href="newsShow.action?newsId=${imageNews.newsid }" target="_blank" title="${imageNews.title }">${fn:substring(imageNews.title,0,18) }</A></P>
					</c:forEach>
  ...
  <LI>5</LI></UL></DIV>
		...
		<div class="newsHeader_list">
			<div class="newsHeader">
				<h3><a href="newsShow.action?newsId=${headNews.newsid }" title="${headNews.title }">${fn:substring(headNews.title,0,10) }</a></h3>
				<p>${fn:substring(headNews.content,0,40) }...<a href="newsShow.action?newsId=${headNews.newsid }" >[查看全文]</a></p>
			</div>
			<div class="currentUpdate">
				...
				<table width="100%">
					<c:forEach var="newestNews" items="${newestNewsList }" varStatus="status">
						...
						<td width="50%">
							<a href="newsShow.action?newsId=${newestNews.newsid }" title="${newestNews.title }">${fn:substring(newestNews.title,0,12) }</a>
						</td>
			...
			<div class="datas">
				<ul>
					<c:forEach var="hotSpotNews" items="${hotSpotNewsList }">
						<li><a href="newsShow.action?newsId=${hotSpotNews.newsid }" title="${hotSpotNews.title }">${fn:substring(hotSpotNews.title,0,15) }</a></li>
					</c:forEach>
				</ul>
			</div>
		</div>
	...
	<c:forEach var="indexNews" items="${allIndexNews }" varStatus="oneStatus">
		<c:if test="${oneStatus.first }">
			...
			<div class="data_list news_list" >
				<div class="dataHeader">${indexNews.typeName }<span class="more"><a href="newsList.action?typeId=${indexNews.typeid }">更多...</a></span></div>
				<div class="datas">
					<ul>
		</c:if>
		<li><fmt:formatDate value="${indexNews.publishdate }" type="date" pattern="MM-dd"/>&nbsp;<a href="newsShow.action?newsId=${indexNews.newsid }" title="${indexNews.title }">${fn:substring(indexNews.title,0,18) }</a></li>
