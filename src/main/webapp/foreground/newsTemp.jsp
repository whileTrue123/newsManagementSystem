			...
			<div class="datas">
				<ul>
					<c:forEach var="newestNews" items="${newestNewsList }">
						<li><a href="newsShow.action?newsId=${newestNews.newsid }" title="${newestNews.title }">${fn:substring(newestNews.title,0,22) }</a></li>
					</c:forEach>
				</ul>
			</div>
		</div>
			...
			<div class="datas">
				<ul>
					<c:forEach var="hotNews" items="${hotNewsList }">
						<li><a href="newShow?newsId=${hotNews.newsid }" title="${hotNews.title }">${fn:substring(hotNews.title,0,22) }</a></li>
					</c:forEach>
				</ul>
			</div>
		</div>
